	...
	public boolean determineCommand(String line)
	{
		try
		{
			...
			if(lower.equals("s") || lower.equals("status") || lower.startsWith("display_status") || lower.startsWith("display status"))
				...
				driver.download(line.substring(8), true, Wreaper.log_download_successful, Wreaper.log_error);
			
			else if(lower.startsWith("merge") || lower.startsWith("concat") || lower.startsWith("cat") || lower.startsWith("join") || lower.startsWith("amalg"))
				driver.merge_directory_of_text_files_into_single_file();
			
			else if(lower.startsWith("split"))
				driver.split_text_files();
			
			else if(lower.startsWith("wreaper"))
			{
				Wreaper wreaper = new Wreaper(line.substring(7), true, true, null);
			}
			
			else if(lower.startsWith("get"))
			{
				Wreaper wreaper = new Wreaper(line.substring(3), true, true, null);
			}
			...
			else if(lower.startsWith("wreap"))
			{
				Wreaper wreaper = new Wreaper(line.substring(5), true, true, null);
			}
			...
			else if(lower.startsWith("is ipv4"))
				driver.directive(driver.is_ipv4(line.substring(7)) + "\n");
