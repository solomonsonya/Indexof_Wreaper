...
public class Log extends Thread implements Runnable, ActionListener
{
	...
	public volatile boolean use_log_in_path = true;
	...
	/**
	 * to be used for log directly
	 * USE_LOG_IN_PATH indicates if we should create a "log" folder within the path of the log file
	 * 
	 * e.g. log_download_successful = new Log(fleTopFolder.getCanonicalPath(), "_log_download_successful", false);
	 * 
	 * @param top_folder_name
	 * @param log_name
	 */
	public Log(String top_folder_name, String log_name, boolean USE_LOG_IN_PATH)
	{
		try
		{
			LOG_FILE_NAME = log_name;
			...
			use_log_in_path = USE_LOG_IN_PATH;
		...
		try
		{
			...
			if(fleLogDirectory == null || !fleLogDirectory.exists() || !fleLogDirectory.isDirectory())
			{
				...
				if(fleLogDirectory.getCanonicalPath().endsWith(File.separator))
				{
					...
					{
						if(TOP_FOLDER_NAME != null && !TOP_FOLDER_NAME.trim().equals(""))
							fleLogDirectory = new File(fleLogDirectory.getCanonicalPath() + Driver.NAME + File.separator + TOP_FOLDER_NAME.trim());
						else
							fleLogDirectory = new File(fleLogDirectory.getCanonicalPath() + Driver.NAME + File.separator);	
					}
				...
				else
				{
					...
					{
						if(TOP_FOLDER_NAME != null && !TOP_FOLDER_NAME.trim().equals(""))
							fleLogDirectory = new File(fleLogDirectory.getCanonicalPath() + File.separator + Driver.NAME + File.separator + TOP_FOLDER_NAME.trim());
						else
							fleLogDirectory = new File(fleLogDirectory.getCanonicalPath() + File.separator + Driver.NAME + File.separator );
					}
	...
	public boolean close_and_open_log_file(boolean open_file)
	{
		try
		{
			try {	this.queue.clear();	}	catch(Exception e){}
			...
			//attempt to open the file
			if(open_file && driver.isWindows && fleLogFile.exists())
			{
				try	{	Process p = Runtime.getRuntime().exec("explorer.exe " + fleLogFile.getCanonicalPath());	}	catch(Exception e){}
			}
			...
			driver.sop("\n\nComplete. Log file written to --> " + fleLogFile);
